...
{
    if (builder.Environment.IsEnvironment("Testing")) options.UseInMemoryDatabase("sim-tests");
...
public sealed class SimDbContext(DbContextOptions<SimDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder) { modelBuilder.Entity<Customer>().Property(x => x.PhoneNumbers).HasColumnType("text[]"); modelBuilder.Entity<Order>().Property(x => x.Revenue).HasPrecision(18, 2); }
}
